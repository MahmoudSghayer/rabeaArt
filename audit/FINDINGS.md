...
| AVL-02 | Availability | No restore has ever been tested | CRITICAL | Critical | An untested backup is a hypothesis, not a recovery plan | **Tooling ready** — `docs/verify-restore.sql` turns validation into one paste; the restore itself is still manual | `docs/verify-restore.sql` |
| DB-02 | Database | Only recovery artifact in-repo is `rollback.sql`, which *drops everything*, with no guard | CRITICAL | High | One paste into the wrong SQL editor destroys production | ✅ **FIXED** — now aborts unless explicitly armed | `docs/rollback.sql:31` |
...
| DB-06 | Database | `order_ref_seq` + stock CHECK exist only in raw migration SQL, not `schema.prisma` | WARNING | Medium | A `prisma db push` silently breaks order submission | **Mitigated** — prominent warning in `schema.prisma`; `verify-restore.sql` checks 2 & 4 detect loss | `prisma/schema.prisma:1-24` |
