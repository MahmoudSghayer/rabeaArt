...
| CACHE-02 | Caching | Nothing is prerendered — about/contact/legal have zero dynamic inputs yet SSR per hit | WARNING | Medium | Wasted compute on pure-content pages | Open (roadmap P3) | `.next/prerender-manifest.json` |
...
| SEO-01 | Frontend | No `sitemap.ts`, `robots.ts`, manifest, or JSON-LD structured data | WARNING | Medium | No crawl map and no `Product` rich results for an e-commerce site | Open (roadmap P3) | `src/app/` |
